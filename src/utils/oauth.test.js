...
const OAUTH_CONSUMER_SECRET =
  '1MGo2oEgLMS9Phqu3nxcnH5zzL1gWx36UMo0cf679wJKAk1TUy';
...
test('checks oauth base string', () => {
  ...
  const SORTED_OAUTH_AND_REQUEST_PARAMS = `id=${encodeURIComponent(
    REQUEST_IDS_PARAM,
  )}&oauth_consumer_key=${OAUTH_CONSUMER_KEY}&oauth_nonce=${OAUTH_NONCE}&oauth_signature_method=${OAUTH_SIGNATURE_METHOD}&oauth_timestamp=${OAUTH_TIMESTAMP}&oauth_token=${OAUTH_ACCESS_TOKEN}&oauth_version=${OAUTH_VERSION}`;
  // eslint-disable-next-line prettier/prettier
  const baseStringSample = `${REQUEST_METHOD}&${encodeURIComponent(
    REQUEST_URL,
  )}&${encodeURIComponent(SORTED_OAUTH_AND_REQUEST_PARAMS)}`;
